...
export function createAppRouter(queryClient: QueryClient, history?: RouterHistory) {
  ...
  onSessionExpired(() => {
    ...
    whenSettled(router, () => {
      // Released as soon as this can act, so a signal that arrives later is never dropped by a
      // latch left standing; the navigation below takes it again for its own duration.
      redirecting = false;
      const { pathname, href } = router.state.location;
      if (pathname === '/login') return;
      // Expiry cannot be a portal-wide redirect: most of this portal is public, and the client
      // cannot tell "your session ended" from "you never had one". So a reader on a public page
      // stays on it and only forgets the profile — otherwise the chrome keeps offering an
...
declare module '@tanstack/react-router' {
  ...
  /** Set by the routes a reader cannot stay on without a session, and read on expiry above. */
