...
const workModes = Object.keys(WORK_MODE_LABELS) as [WorkMode, ...WorkMode[]];
...
export const jobFiltersSchema = z.object({
  ...
  mode: z.enum(workModes).optional().catch(undefined),
});
...
export function asWorkMode(chosen: string | null): WorkMode | undefined {
  return jobFiltersSchema.shape.mode.parse(chosen);
}

export const NO_FILTERS: JobFilters = {};

export function isFiltered(filters: JobFilters): boolean {
  return Boolean(filters.q || filters.location || filters.type || filters.mode);
}

export function browseQuery(filters: JobFilters) {
  return {
    q: filters.q,
    location_key: filters.location,
    employment_type: filters.type,
    work_mode: filters.mode,
  };
}
