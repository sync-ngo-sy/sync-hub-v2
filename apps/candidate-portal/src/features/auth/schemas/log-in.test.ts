...
describe('the log-in schema', () => {
  it('accepts a filled-in form', () => {
    expect(
      logInSchema.safeParse({ email: 'lina@example.test', password: 'correct-horse' }).success,
    ).toBe(true);
  });
  ...
  it('rejects something that is not an email address', () => {
    expect(errorFor('email', { email: 'lina', password: 'correct-horse' })).toBe(
      'Enter a valid email address.',
    );
  });

  it('asks for a password', () => {
    expect(errorFor('password', { email: 'lina@example.test', password: '' })).toBe(
      'Enter your password.',
    );
  });
});
