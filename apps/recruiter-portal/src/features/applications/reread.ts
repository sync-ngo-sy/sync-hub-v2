...
export function onApplication(applicationId: string) {
  return { params: { path: { application_id: applicationId } } };
}

export function applicationReview(applicationId: string) {
  return api.queryOptions('get', APPLICATION_PATH, onApplication(applicationId));
}

export function matchAssessment(applicationId: string) {
  return api.queryOptions('get', ASSESSMENT_PATH, onApplication(applicationId));
}
...
function everyNotesReading(applicationId: string) {
  return api.queryOptions('get', NOTES_PATH, onApplication(applicationId)).queryKey;
}

function everyTagsReading(applicationId: string) {
  return api.queryOptions('get', TAGS_PATH, onApplication(applicationId)).queryKey;
}
...
export function useRereadApplicationTags(applicationId: string) {
  const queryClient = useQueryClient();
  return () => queryClient.invalidateQueries({ queryKey: everyTagsReading(applicationId) });
}
