...
import {
  ...
  matchedCard,
  pooledCard,
} from '@/features/candidates/candidate';
import {
  CandidateOutOfReach,
  CandidateViewPage,
} from '@/features/candidates/components/candidate-view-page';
import { readDirectoryHits } from '@/features/candidates/hooks/use-candidate-directory';
...
import { candidateOrderFrom, candidateSearchParams, filtersFrom } from './-candidate-search-params';
...
export const Route = createFileRoute('/_workspace/candidates_/$candidateId')({
  ...
  loader: async ({ context, params, deps }) => {
    ...
    const [hits, listed, pool] = await Promise.all([
      readSearchHits(context.queryClient, filters),
      readDirectoryHits(context.queryClient, filters, candidateOrderFrom(deps)),
    ...
    const person = listed.find((one) => one.candidate_id === params.candidateId);
    if (person) return { card: listedCard(person), evidence: null };
