...
function control(name: 'Screening' | 'Pipeline') {
  return within(screen.getByRole('tablist', { name }));
}
...
describe("a Job's Applications tab", () => {
  ...
  it('sends both filters to the API at once and leaves them in the address bar', async () => {
    ...
    await user.click(await control('Screening').findByRole('tab', { name: 'Review required' }));
    await waitFor(() =>
      expect(router.state.location.search).toEqual({ qualification: 'review_required' }),
    );

    await user.click(control('Pipeline').getByRole('tab', { name: 'Shortlisted' }));
    await waitFor(() =>
  ...
  it('reproduces the list a shared link was copied from', async () => {
    ...
    await renderApp(`/jobs/${JOB.id}?status=rejected&qualification=disqualified`);

    expect(await screen.findByText('Carla Rizk')).toBeVisible();
    expect(screen.queryByText('Amal Haddad')).toBeNull();
    expect(control('Screening').getByRole('tab', { name: 'Disqualified' })).toHaveAttribute(
      'data-active',
    );
    expect(control('Pipeline').getByRole('tab', { name: 'Rejected' })).toHaveAttribute(
      'data-active',
    );
  ...
  it('drops a filter the platform cannot honour rather than failing the page', async () => {
    ...
    await renderApp(`/jobs/${JOB.id}?status=on-a-yacht`);
  ...
  it('stands a skeleton in the table while the first page is on the wire', async () => {
    ...
    server.use(
      ...
      http.get(PATH, async ({ response }) => {
        await held;
        return response(200).json({ items: [AMAL], next_cursor: null });
      }),
    );
  ...
  it('says a filtered view is empty because of the filters, and offers to drop them', async () => {
    ...
    const { router, user } = await renderApp(`/jobs/${JOB.id}?status=hired&qualification=pending`);
  ...
  it('carries the filters across a trip to another tab', async () => {
    ...
    const { router, user } = await renderApp(`/jobs/${JOB.id}?qualification=qualified`);

    expect(await screen.findByText('Amal Haddad')).toBeVisible();

    await user.click(screen.getByRole('tab', { name: 'Tracked links' }));
    await waitFor(() =>
      expect(router.state.location.search).toEqual({ tab: 'links', qualification: 'qualified' }),
    );

    await user.click(screen.getByRole('tab', { name: 'Applications' }));

    expect(await screen.findByText('Amal Haddad')).toBeVisible();
    expect(control('Screening').getByRole('tab', { name: 'Qualified' })).toHaveAttribute(
      'data-active',
    );
  });
});
