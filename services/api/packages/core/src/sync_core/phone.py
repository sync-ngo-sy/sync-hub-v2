...
what it sends, so a number the field accepted is never one the API then refuses — and a number
that arrives another way meets exactly the field's standard.
"""
...
def read(number: str, country: str | None = None) -> Phone | None:
    ...
    if home is None or (country is not None and home != country):
        return None
