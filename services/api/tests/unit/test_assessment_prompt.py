...
def test_the_employment_type_reads_as_english_rather_than_as_its_stored_value() -> None:
    """The model reads prose, and `full_time` is not prose. The words are the same ones the
    portals put on screen, so the document did not change when the column became an enum."""
    document = as_document(MatchRequest(job=A_JOB, application=AN_APPLICATION))
